...
enum ApplicationStatus: String, Codable, CaseIterable, Identifiable {
    ...
    // Fallback decoder so unknown DB values don't crash the app
    init(from decoder: Decoder) throws {
        let raw = try decoder.singleValueContainer().decode(String.self)
        self = ApplicationStatus(rawValue: raw) ?? .unknown
    }
...
struct Application: Codable, Identifiable {
    ...
    var formattedDate: String {
        // Try full datetime first (e.g. "2025-06-22T03:03:23+00:00"), then date-only
        let isoFull = ISO8601DateFormatter()
        isoFull.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        let isoDate = ISO8601DateFormatter()
        isoDate.formatOptions = [.withFullDate]
        let date = isoFull.date(from: date_applied)
                ?? isoDate.date(from: date_applied)
        guard let date else { return date_applied }
