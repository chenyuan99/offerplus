-- Migration: Create H1B Statistics Functions
...
BEGIN
  ...
  SELECT ROUND(AVG(salary), 0) INTO avg_salary FROM filtered_data WHERE salary > 0;
  
  -- Calculate median salary
  SELECT ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY salary), 0) 
